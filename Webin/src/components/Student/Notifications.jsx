...
const TYPE_META = {
  ...
  APPLICATION_REVIEWED:    { icon: "👁", color: "text-sky-400",     bg: "bg-sky-500/10 border-sky-500/25",         dot: "bg-sky-400"     },
  ...
  INTERVIEW_SCHEDULED:     { icon: "🎤", color: "text-violet-400",  bg: "bg-violet-500/10 border-violet-500/25",   dot: "bg-violet-400"  },
  ...
  return (
    <div className="max-w-2xl space-y-5">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-base font-black text-slate-100" style={{fontFamily:"'Syne',sans-serif"}}>Notifications</h2>
          <p className="text-xs text-slate-600 font-mono mt-0.5">{unread} unread · Application & interview updates</p>
        </div>
        <div className="flex items-center gap-2">
          {unread > 0 && (
            <button onClick={markAll}
              className="text-xs font-bold text-sky-400 hover:text-sky-300 border border-sky-500/25 bg-sky-500/10 px-3 py-1.5 rounded-lg transition-colors"
          ...
          {types.map(t => {
            ...
            return (
              <button key={t} onClick={() => setFilter(t)}
                className={`px-3 py-1.5 rounded-xl text-[11px] font-bold border transition-all ${filter === t ? "bg-sky-500/15 border-sky-500/35 text-sky-400" : "bg-slate-900 border-slate-800 text-slate-600 hover:border-slate-700 hover:text-slate-400"}`}
                style={{fontFamily:"'Syne',sans-serif"}}>
                ...
                <div key={n.notification_id} onClick={() => markRead(n.notification_id)}
                  ...
                  <div className="flex-1 min-w-0 pr-4">
                    ...
                    <div className="flex items-center gap-3 mt-2 flex-wrap">
                      ...
                      {n.company_name && <><span className="text-slate-800">·</span><span className="text-[10px] text-slate-500 font-mono">{n.company_name}</span></>}
